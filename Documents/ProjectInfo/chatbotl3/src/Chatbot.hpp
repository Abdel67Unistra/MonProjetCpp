...
private:
    ...
    std::map<std::string, std::map<std::string, bool>> factual_knowledge;  ///< Base de connaissances factuelle pour les mots
    std::map<std::string, std::vector<std::string>> synonyms;              ///< Synonymes et associations pour les mots
    ...
    /**
     * @brief Calcule la similarité entre deux chaînes de caractères (0 à 1)
     * @param str1 Première chaîne
     * @param str2 Seconde chaîne
     * @return Score de similarité entre 0 (différent) et 1 (identique)
     */
    ...
    /**
     * @brief Vérifie la cohérence logique entre une question et une réponse
     * @param question La question à vérifier
     * @param proposed_answer La réponse proposée
     * @return true si la cohérence logique est respectée, false sinon
     */
    bool checkLogicalConsistency(const std::string& question, const std::string& proposed_answer);
...
public:
    ...
    /**
     * @brief Initialise le jeu en choisissant un mot aléatoire
     */
    void thinkOfWord();
    
    /**
     * @brief Récupère le mot choisi par l'IA
     * @return Le mot choisi
     */
    std::string getChosenWord() const { return chosen_word; }
    
    /**
     * @brief Récupère la catégorie du mot choisi
     * @return La catégorie du mot
     */
    std::string getChosenCategory() const { return chosen_category; }
    
    /**
     * @brief Récupère la dernière erreur survenue
     * @return Le message d'erreur
     */
    std::string getLastError() const { return last_error; }
    
    /**
     * @brief Récupère la clé API
     * @return La clé API
     */
    std::string getApiKey() const { return api_key; }
    
    /**
     * @brief Récupère le nom du modèle
     * @return Le nom du modèle
     */
    std::string getModel() const { return model; }
    
    /**
     * @brief Définit le mode débogage
     * @param debug_mode true pour activer le débogage, false sinon
     */
    ...
    /**
     * @brief Configure le mode où l'IA devine le mot du joueur
     */
